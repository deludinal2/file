
function FindProxyForURL(url, host) {
  // Variables defined to proxy or send direct
  var proxy = 'PROXY relay.lsaccess.me:62312';
  var direct = 'DIRECT';
  var lowerUrl = url.toLowerCase();
  // Internal IP Ranges
  if (
    isInNet(host, "127.0.0.0", "255.0.0.0") ||
    isInNet(host, "10.0.0.0", "255.0.0.0") ||
    isInNet(host, "172.16.0.0", "255.240.0.0") ||
    isInNet(host, "192.168.0.0", "255.255.0.0") ||
    isInNet(host, "169.254.0.0", "255.255.0.0")
  ) {
    return direct;
  }
  // Send Plain Hostnames DIRECT
  if (isPlainHostName(host))
    return direct;
  // Send FTP traffic DIRECT
  if (lowerUrl.substring(0, 4) === 'ftp:') {
    return direct;
  }
  if (lowerUrl.startsWith('wss:')) {
    return direct;
  }
  if (lowerUrl.startsWith('ws:')) {
    return direct;
  }
  // papercut printer discovery should go direct
  if (/^rpc\.pc-printer-discovery(?:-\d+)?$/.test(host)) {
    return direct;
  }
  // Entries to cover subdomains
  var no_proxy_subs = Array(
	".savvas.com",
	".amazon.com",
	".savvasrealize.com",
	".savvastexas.com",
	".mysavvastraining.com",
	".classviewappstore.com",
	".pk12ls.com",
	".apps.canva-apps.com",
	".hjedesign.com",
	".myhjyearbook.com",
	".hjeshare.com",
	".herffjones.com",
	".cloudflareinsights.com",
	".gstatic.com",
	".sentry.io",
	".savvaseasybridge.com",
	".googleapis.com",
	".ssl.gstatic.com",
	".zoom.us",
	".webex.com",
	".ocsp.apple.com",
	".renaissance-go.com",
	".cricut.com",
	".outlook.office365.com",
	".r4.res.office365.com",
	".renlearn.com",
	".renaissance.com",
	".classroom.google.com",
	".testnav.com",
	".renlearnrp.com",
	".zoom.com",
	".pearsontestcontent.com",
	".meetzoom.net",
	".tylertech.com",
	".drcedirect.com",
	".pearson.com",
	".starttest.com",
	".kiteaai.org",
	".khanacademy.org",
	".infinitecampus.org",
	".comodoca.com",
	".support.hp.com",
	".youtube-nocookie.com",
	".makerbot.com",
	".drc-centraloffice.com",
	".edpuzzle.com",
	".lsrelay-reports-production.s3-us-west-2.amazonaws.com",
	".pearsonvue.com",
	".sophos.com",
	".unsplash.com",
	".donjohnston.net",
	".userback.io",
	".fonts.gstatic.com",
	".media.pk12ls.com",
	".savvascmg.com",
	".tokenex.com",
	".ecollege.com",
	".chompchomp.com",
	".easybridge-dashboard-web.savvaseasybridge.com",
	".pescirrus.com",
	".ushistory.org",
	".live.com",
	".office.net",
	".office.com",
	".sharepoint.com",
	".manage.microsoft.com",
	".microsoft.com",
	".microsoftonline.com",
	".ets.org",
	".sso.rumba.pk12ls.com",
	".cat.easybridge.pk12ls.com",
	".prodigygames.com",
	".chime.aws",
	".adamexam.com",
	".act.org",
	".thawte.com",
	".usertrust.com",
	".cloudfront.net",
	".mypearson.com",
	".registration.mypearson.com",
	".www.gstatic.com",
	".collegeboard.org",
	".gimkitconnect.com",
	".gimkit.com",
	".logmeinrescue.com",
	".logmein.com",
	".canva.com",
	".logmein123.com",
	".a.manage.microsoft.com",
    ".instagram.com",
	".dm.microsoft.com",
	".apps.mzstatic.com",
	".updates.cdn-apple.com",
	".cert-root-public-keys-production.s3.us-west-2.amazonaws.com",
	".i-ready.com",
	".slack-msgs.com:443",
	".microsoft.com",
	".msftconnecttest.com",
	".apple.com",
	".ably.io",
	".twilio.com",
	".icloud.com",
	".lsmdm.com",
	".turn.anyfirewall.com",
	".q73sd1i7s5.execute-api.us-west-2.amazonaws.com",
	".d5h6hjc5h3.execute-api.us-west-1.amazonaws.com",
	".p7nvu5it0k.execute-api.us-west-2.amazonaws.com",
	".lsrelay-extensions-production.s3.amazonaws.com",
	".lsrelay-extensions-production.s3-us-west-2.amazonaws.com",
	".oscp.apple.com",
	".mzstatic.com",
	".digicert.com",
	".lsmdm.com",
	".lsfilter.com",
	".relay.school",
	".lightspeedsystems.com",
	".lightspeedsystems.app",
	".stagingls.io",
	".developmentls.io"
  );
  // Entries to cover exact match domains
  var no_proxy_exact = Array(
	"savvas.com",
	"savvasrealize.com",
	"savvastexas.com",
	"mysavvastraining.com",
	"classviewappstore.com",
	"pk12ls.com",
	"apps.canva-apps.com",
	"hjedesign.com",
	"myhjyearbook.com",
	"hjeshare.com",
	"herffjones.com",
	"cloudflareinsights.com",
	"gstatic.com",
	"sentry.io",
	"savvaseasybridge.com",
	"googleapis.com",
	"ssl.gstatic.com",
	"zoom.us",
	"webex.com",
	"ocsp.apple.com",
	"renaissance-go.com",
	"cricut.com",
	"outlook.office365.com",
	"r4.res.office365.com",
	"renlearn.com",
	"renaissance.com",
	"classroom.google.com",
	"testnav.com",
	"renlearnrp.com",
	"zoom.com",
	"pearsontestcontent.com",
	"meetzoom.net",
	"tylertech.com",
	"drcedirect.com",
	"pearson.com",
	"starttest.com",
	"kiteaai.org",
	"khanacademy.org",
	"infinitecampus.org",
	"comodoca.com",
	"support.hp.com",
	"youtube-nocookie.com",
	"makerbot.com",
	"drc-centraloffice.com",
	"edpuzzle.com",
	"lsrelay-reports-production.s3-us-west-2.amazonaws.com",
	"pearsonvue.com",
	"sophos.com",
	"unsplash.com",
	"donjohnston.net",
	"userback.io",
	"fonts.gstatic.com",
	"media.pk12ls.com",
	"savvascmg.com",
	"tokenex.com",
	"ecollege.com",
	"chompchomp.com",
	"easybridge-dashboard-web.savvaseasybridge.com",
	"pescirrus.com",
	"ushistory.org",
	"live.com",
	"office.net",
	"office.com",
	"sharepoint.com",
	"manage.microsoft.com",
	"microsoft.com",
	"microsoftonline.com",
	"ets.org",
	"sso.rumba.pk12ls.com",
	"cat.easybridge.pk12ls.com",
	"prodigygames.com",
	"chime.aws",
	"adamexam.com",
	"act.org",
	"thawte.com",
	"usertrust.com",
	"cloudfront.net",
	"mypearson.com",
	"registration.mypearson.com",
	"www.gstatic.com",
	"collegeboard.org",
	"gimkitconnect.com",
	"gimkit.com",
	"logmeinrescue.com",
	"logmein.com",
	"canva.com",
	"logmein123.com",
	"a.manage.microsoft.com",
	"dm.microsoft.com",
	"apps.mzstatic.com",
	"updates.cdn-apple.com",
	"cert-root-public-keys-production.s3.us-west-2.amazonaws.com",
	"i-ready.com",
	"slack-msgs.com:443",
	"microsoft.com",
	"msftconnecttest.com",
	"apple.com",
	"ably.io",
	"twilio.com",
	"icloud.com",
	"lsmdm.com",
	"turn.anyfirewall.com",
	"q73sd1i7s5.execute-api.us-west-2.amazonaws.com",
	"d5h6hjc5h3.execute-api.us-west-1.amazonaws.com",
	"p7nvu5it0k.execute-api.us-west-2.amazonaws.com",
	"lsrelay-extensions-production.s3.amazonaws.com",
	"lsrelay-extensions-production.s3-us-west-2.amazonaws.com",
	"oscp.apple.com",
	"mzstatic.com",
	"digicert.com",
	"lsmdm.com",
    "instagram.com",
	"lsfilter.com",
	"relay.school",
	"lightspeedsystems.com",
	"lightspeedsystems.app",
	"stagingls.io",
	"developmentls.io"
  );
  // Match the above lists to send direct
  for (var iter = 0; iter < no_proxy_subs.length; ++iter) {
    if (dnsDomainIs(host, no_proxy_subs[iter])) {
      return direct;
    }
  }
  for (var iter = 0; iter < no_proxy_exact.length; ++iter) {
    if (localHostOrDomainIs(host, no_proxy_exact[iter])) {
      return direct;
    }
  }
  // DEFAULT RULE: All other traffic sent to proxy.
  return proxy;
}
